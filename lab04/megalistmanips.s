...
mapLoop:
    lw t1, 0(s0)        # load the address of the array of current node into t1  1st mistake
    ...
    jalr s1             # call the function on that value.  2nd mistake
    ...
    addi t0, t0, 1      # increment the count 3rd mistake
    bne t0, t2, mapLoop # repeat if we haven't reached the array size yet

    lw a0, 8(s0)        # load the address of the next node into a0 4th mistake
    la s1, mystery      # put the address of the function back into a1 to prepare for the recursion 5th mistake
    jal  map            # recurse
done:
    lw s0, 8(sp)
    lw s1, 4(sp)
